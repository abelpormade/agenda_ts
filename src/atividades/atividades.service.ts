...
export class AtividadesService {
  ...
  async findOne(id: number) {
    const EncontrarPorId= await this.atividadeRepository.findOne({ where: { id_atividade: id } });
  ...
  async remove(id: number) {
    const atividade = await this.atividadeRepository.findOne({ where: { id_atividade: id } });
